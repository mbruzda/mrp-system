    ...
    public class MRPlvl1 : IERP
    {
        ...
        public void FillTable()
        {
            ...
            { 
                ...
                //PlannedRelease
                for (int i = 0; i < 10; i++) if (MRPData.NetRequirements[i] > 0) MRPData.PlannedRelease[i] = MRPData.LotSize > MRPData.NetRequirements[i] ? MRPData.LotSize : MRPData.LotSize * 2;
                //PlannedReceipt
                for (int i = 0; i < 10; i++) if (MRPData.PlannedRelease[i] > 0) MRPData.PlannedReceipt[i - MRPData.RealizationTime] = MRPData.PlannedRelease[i];
